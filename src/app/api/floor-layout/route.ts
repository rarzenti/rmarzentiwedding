...
export async function GET() {
  try {
    ...
    // Handle both old format (just positions) and new format (with tableCount)
    if (parsed.positions !== undefined || parsed.tableCount !== undefined) {
    ...
    return NextResponse.json({ layout: parsed, tableCount: 20 });
  } catch {
    // Return defaults if file doesn't exist or can't be read
    return NextResponse.json({ layout: null, tableCount: 20 });
  }
}
